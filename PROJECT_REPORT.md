...
2. **AI-Assisted Event Creation** — Reduce manual effort through banner image analysis (OCR) and conversational event creation powered by Google Gemini.
3. **Secure Online Payments** — Integrate SSLCommerz payment gateway (bKash, Nagad, Visa, Mastercard) with configurable refund policies and deferred registration (no DB entry until payment succeeds).
4. **Automated Communication** — Send email notifications for event approvals, registration status changes, and event cancellations via SMTP.
5. **Multi-Role Access Control** — Support five roles (Super Admin, Admin, Institution, Organizer, Participant), each with a dedicated dashboard and appropriate permissions.
6. **External Event Aggregation** — Crawl third-party websites to discover and import external event listings.

### Overview

The application is architected as three loosely coupled services:
...
- A **Python FastAPI microservice** providing AI capabilities (Gemini chat, EasyOCR banner analysis, web crawling)
...
| FR-04 | AI-Assisted Creation | Users can upload a banner/poster image for AI-based text extraction (EasyOCR + Gemini). A conversational mode allows creating events through natural language prompts. | Medium |
| FR-05 | Event Email Verification | If the event's contact email differs from the creator's account email, an approval token is generated and an approval email is sent to the contact. The event remains in "pending approval" until the contact approves via the email link. | High |
| FR-06 | Event Approval by Admin | Admins can review pending events and approve or reject them with reasons. Creators receive email notifications of the decision. | High |
| FR-07 | Registration Form Builder | Organizers build custom registration forms with a drag-and-drop form builder supporting text, email, phone, dropdown, checkbox, and file upload fields. Both individual and team-based templates are supported. | High |
| FR-08 | Payment Configuration | Organizers can configure paid events with fee amount, currency, and refund policy (full refund, partial refund, no refund, or custom). | High |
| FR-09 | Event Discovery | Participants browse/search published events with filtering by category, date, and status on an explore page. Events display in cards with detailed view pages. | High |
| FR-10 | Free Event Registration | For free events, participants submit the registration form and a participant record is created immediately with status "pending". | High |
| FR-11 | Paid Event Registration | For paid events, form data is stored in the transaction record and the user is redirected to SSLCommerz. The participant record is only created after successful payment (via IPN callback). If the user presses back or payment fails, no registration is saved. | High |
| FR-12 | Participant Self-Cancellation | Participants can cancel their own registration. Refunds are processed according to the event's configured refund policy (full, partial, none, or custom — where custom creates a pending refund request for the organizer). | Medium |
| FR-13 | Organizer Participant Management | Organizers view pending/approved/rejected participants. They can approve or reject applications. Rejection triggers automatic refund if the participant had paid. Email notifications are sent for both decisions. | High |
| FR-14 | Event Cancellation | Event creators can cancel their event. This cancels all participant registrations, processes full refunds for all paid participants (regardless of refund policy), and sends cancellation emails. | High |
| FR-15 | Bookmarks & Calendar | Participants bookmark events and view them in a calendar with day/week views (FullCalendar). | Low |
| FR-16 | Transaction History | Participants view their payment history. Organizers and institutions have payment dashboards showing revenue per event with refund tracking. | Medium |
| FR-17 | AI Chatbot | A floating chatbot widget provides role-aware assistance about the platform, powered by Gemini with conversation context tracking and intent detection. | Medium |
| FR-18 | Web Crawler | Admins add external event source URLs. The system crawls these using curl_cffi and BeautifulSoup to extract and display event listings. | Low |
| FR-19 | Role Management | Super admins can assign or revoke any role for any user. | High |
| FR-20 | User Profiles | All users have editable profiles with profile picture and banner (Cloudinary-hosted). Institution profiles display verification status and badge. | Medium |

### Non-Functional Requirements

| ID | Category | Requirement |
|----|----------|-------------|
| NFR-01 | **Security** | Helmet.js for HTTP security headers; Firebase Auth for identity management; rate limiting (100 req/15min general, 10 req/15min for auth, 5 req/15min for AI); CORS configuration. |
...
| NFR-03 | **Scalability** | Stateless Express API supports horizontal scaling; AI microservice runs independently on separate port; Supabase provides managed PostgreSQL with connection pooling. |
...
graph TD
    ...
    B --> C{"Choose creation\nmethod"}
    C -->|"Manual"| D["Fill all fields\nmanually"]
    C -->|"AI Banner"| E["Upload poster image\n→ EasyOCR + Gemini\nextracts details"]
    C -->|"AI Chat"| F["Describe event in\nnatural language\n→ Gemini structures data"]
    D --> G["Click Submit"]
    E --> G
    F --> G
...
| Google Generative AI | 0.24.1 | Gemini API SDK for AI chat in Node.js |
...
| Package | Version | Purpose |
|---------|---------|---------|
| FastAPI | ≥ 0.100.0 | Async Python web framework with auto-generated docs |
| Uvicorn | ≥ 0.23.0 | ASGI server running the FastAPI application |
| PyTorch | ≥ 2.0.0 | Deep learning framework for model inference |
| EasyOCR | ≥ 1.7.0 | Optical Character Recognition from banner/poster images |
| OpenCV | ≥ 4.8.0 | Image preprocessing (resize, contrast, thresholding) |
| Pillow | ≥ 9.5.0 | Image format handling and manipulation |
| NumPy | ≥ 1.24.0 | Array operations for image data processing |
| BeautifulSoup4 | ≥ 4.12.0 | HTML parsing for web crawler event extraction |
| curl_cffi | ≥ 0.5.10 | HTTP client with browser TLS fingerprint impersonation |
...
| **Google Gemini** | LLM for chatbot responses, event data structuring from OCR text, and conversational event creation |
...
|-----------|----------------------|
| Development Machine | 8 GB RAM, modern multi-core CPU, Node.js 18+, Python 3.10+ |
| AI Server | GPU recommended for EasyOCR/PyTorch inference; CPU fallback supported but slower |
...
| TC-09 | AI | Banner analysis | AI server running | Upload event poster image | Event form auto-populated with extracted title, date, venue |
| TC-10 | Registration | Free event — submit form | Event published, free | Fill registration form, submit | Participant record created (`status = pending`) |
| TC-11 | Registration | Paid event — successful payment | Event published, paid | Fill form → redirected to SSLCommerz → complete payment | Participant created only after IPN confirms payment (`payment_status = completed`) |
| TC-12 | Registration | Paid event — press back on gateway | Event published, paid | Fill form → redirected to SSLCommerz → press browser back | No participant record created; user returns to event page with Register button |
| TC-13 | Registration | Paid event — payment fails | Event published, paid | Fill form → payment fails at gateway | User redirected to /payment/fail; no participant record |
| TC-14 | Participant | Self-cancel (free event) | Registered for free event | Click "Cancel Registration" | status → cancelled, no refund needed |
| TC-15 | Participant | Self-cancel (paid, full refund policy) | Registered + paid, full refund | Click "Cancel Registration" | status → cancelled, full refund initiated via SSLCommerz |
| TC-16 | Participant | Self-cancel (paid, no refund policy) | Registered + paid, no refund | Click "Cancel Registration" | status → cancelled, message: "No refund policy" |
| TC-17 | Organizer | Approve participant | Pending participant exists | Click Approve in Participant Management | status → approved, approval email sent |
| TC-18 | Organizer | Reject participant (paid) | Paid pending participant | Click Reject, enter reason | status → rejected, auto-refund initiated, rejection email sent |
| TC-19 | Event Cancel | Cancel event with participants | Event has registered + paid participants | Click Cancel Event, enter reason, confirm | Event status → cancelled, all registrations cancelled, all paid participants refunded, emails sent |
| TC-20 | Bookmark | Bookmark and unbookmark | Logged in as participant | Click bookmark icon → click again | Bookmark saved then removed; reflected in dashboard |
| TC-21 | Chatbot | Ask platform question | AI server running | Type "How do I register for an event?" | Relevant contextual help response |
| TC-22 | Chatbot | Ask out-of-scope question | AI server running | Type "What is the weather?" | Politely declined, redirected to platform topics |
...
| TC-09 | Banner analysis | ✅ Pass | EasyOCR extracts text, Gemini parses into structured fields |
| TC-10 | Free registration | ✅ Pass | Participant record created immediately, toast confirmation shown |
| TC-11 | Paid — success | ✅ Pass | IPN creates participant from stored `pending_registration_data` |
| TC-12 | Paid — back button | ✅ Pass | No orphaned registration; transaction stays as "initiated" |
| TC-13 | Paid — fail | ✅ Pass | Transaction marked failed; user sees error page |
| TC-14 | Self-cancel free | ✅ Pass | Status cancelled; no refund processing triggered |
| TC-15 | Self-cancel full refund | ✅ Pass | SSLCommerz refund initiated; transaction → refunded |
| TC-16 | Self-cancel no refund | ✅ Pass | Cancelled without refund; user sees "no refund policy" message |
| TC-17 | Approve participant | ✅ Pass | Email sent to both account email and form-submitted email |
| TC-18 | Reject paid participant | ✅ Pass | Auto-refund initiated; rejection email sent with reason |
| TC-19 | Cancel event | ✅ Pass | Bulk refund loop processes all paid participants; cancellation emails sent |
| TC-20 | Bookmark toggle | ✅ Pass | Bookmark appears and disappears in participant dashboard |
| TC-21 | Chatbot help | ✅ Pass | Gemini returns role-aware response based on conversation context |
| TC-22 | Chatbot guardrails | ✅ Pass | Off-topic query politely redirected |
...
- The **AI server** requires significant computational resources (GPU for EasyOCR); this increases deployment costs and cold-start times.
...
The system is designed as **three independent services** — a React frontend, a Node.js API, and a Python AI server — each deployable and scalable independently. The Express backend is **stateless** (all state lives in Supabase), enabling horizontal scaling behind a load balancer. Database-heavy operations use **PostgreSQL stored procedures** (e.g., `register_user`, `create_event_with_timeslots`, `approve_participant`) to minimize network round trips and ensure atomicity. All list endpoints implement **server-side pagination** to handle growing data volumes.

### Reusability

Features are encapsulated in **modular route files** (`payment.routes.js`, `registration.routes.js`, `cancel-event.routes.js`, etc.) with corresponding **service layers** (`email.service.js`, `sslcommerz.service.js`), making it straightforward to extract and reuse any module independently. On the frontend, components like `CancelEventModal`, `ChatBot`, `SearchableInstitution`, `ImageUploader`, and `DocumentUploader` are **prop-driven and stateless**, designed for reuse across different dashboard views.

### Energy Efficiency

The AI models (EasyOCR, PyTorch) are **loaded once on server startup** and reused across all requests, avoiding repeated model loading. **API rate limiting** prevents resource-wasting excessive requests. Media delivery through **Cloudinary's CDN** reduces bandwidth by serving assets from edge nodes. The frontend uses **Vite's code splitting** and **React Router lazy loading** to minimize initial bundle size and reduce client-side processing.
...
- **Transparency:** The AI chatbot's scope is clearly bounded to the Event Corner platform; off-topic queries are politely declined. Chat analytics track blocked queries to improve the system without compromising user privacy.
...
- An **Express.js REST API** with 50+ endpoints, modular route architecture, and service-layer separation for payments, email, and AI integration.
- A **Python FastAPI microservice** integrating Google Gemini for conversational event creation, EasyOCR for banner text extraction, and curl_cffi-based web crawling.
...
The system is architected for scalability through its microservice design, maintainability through modular code and SQL migrations, and reliability through dual payment confirmation and comprehensive error handling. Future enhancements could include real-time WebSocket notifications, mobile applications, and expanded AI capabilities for event recommendations.
